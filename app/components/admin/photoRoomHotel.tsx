...
export default function PhotoRoomHotel({
  roomHotelId,
  submitPhotoForm,
}: PhotoRoomHotelProps) {
  ...
  const handleUploadSingleFile = async (file: RcFile, roomHotelId: string) => {
    const formData = new FormData();
    formData.append("file", file);
    const destId = roomHotelId
      ? roomHotelId
      : localStorage.getItem("_roomHotel") || "";
    formData.append("roomHotelId", destId);

    try {
      setUploading(true);
      await roomHotelRepository.api.addPhotoRoomHotel(formData);
  ...
  const handleUploadAllFiles = async () => {
    ...
    for (const file of fileList) {
      await handleUploadSingleFile(file, roomHotelId);
    }
